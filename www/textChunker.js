...
function splitSentencesHelper(sentence) {
  ...
  else if (hasExclamation(sentence)) {
    splitAfterExclamation(sentence);
  }
  else if (hasQuestion(sentence)) {
    splitAfterQuestion(sentence);
  }
	else {
		chunkArray.push(sentence);
	}
...
function hasExclamation(sentence) {
  if (sentence.indexOf('!') !== -1) {
    var exclamationSplit = sentence.split('! ');
    var afterExclamationSplit = commaSplit[1].split(' ');
    if (exclamationSplit.length>2) {
      return true;
    }
    else if (exclamationSplit[0].split(" ").length < 4 && exclamationSplit[1].split(" ").length < 4) {
      return false;
    }
    else {
      return true;
...
function hasQuestion(sentence) {
  if (sentence.indexOf('?') !== -1) {
    var questionSplit = sentence.split('? ');
    var afterQuestionSplit = questionSplit[1].split(' ');
    if (questionSplit.length>2) {
      return true;
    }
    else if (questionSplit[0].split(" ").length < 4 && questionSplit[1].split(" ").length < 4) {
      return false;
    }
    else {
      return true;
...
function splitAfterExclamation(sentence) {
	var exclamationSplit = sentence.split("! ");
	if (exclamationSplit.length > 2) {
		for (var i = 0; i <= exclamationSplit.length - 1; i++) {
	        if (exclamationSplit[i].split(" ").length < 5 && i!==(exclamationSplit.length - 1)) {
	        	exclamationSplit[i+1] = exclamationSplit[i] + ", " + exclamationSplit[i+1];
                exclamationSplit.splice(i, 1);
			}
		}
		if (exclamationSplit.length === 2) {
			chunkArray.push(exclamationSplit[0] + ", ");
			chunkArray.push(exclamationSplit[1]);
		}
	}
	else {
		var firstSegment = exclamationSplit[0] + ",";
		chunkArray.push(firstSegment);
		chunkArray.push(exclamationSplit[1]);
	}
}

function splitAfterQuestion(sentence) {
	var questionSplit = sentence.split("! ");
	if (questionSplit.length > 2) {
		for (var i = 0; i <= questionSplit.length - 1; i++) {
	        if (questionSplit[i].split(" ").length < 5 && i!==(questionSplit.length - 1)) {
	        	questionSplit[i+1] = questionSplit[i] + ", " + questionSplit[i+1];
                questionSplit.splice(i, 1);
			}
		}
		if (questionSplit.length === 2) {
			chunkArray.push(questionSplit[0] + ", ");
			chunkArray.push(questionSplit[1]);
		}
	}
	else {
		var firstSegment = questionSplit[0] + ",";
		chunkArray.push(firstSegment);
		chunkArray.push(questionSplit[1]);
	}
}
